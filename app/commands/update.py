...
def handle_update(ack, say, command):
    ack()
    text = command.get("text", "").strip()
    user_id = command["user_id"]

    if "," not in text:
        say("Usage: `/update Number, Status` — e.g. `/update 1, Interview`\nValid statuses: Applied, Interview, Offer, Rejected, Withdrawn")
        return
    ...
    try:
        ...
        say("First part must be a number. e.g. `/update 1, Interview`")
        return

    new_status = parts[1].strip().capitalize()

    if new_status not in VALID_STATUSES:
        say(f"Invalid status. Choose from: {', '.join(VALID_STATUSES)}")
        return
